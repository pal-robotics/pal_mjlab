...
print("=" * 80)
print(f"Ranges of observations across {env_cfg.scene.num_envs} simulations after 1 step:")
print("=" * 80)
...
for group_name in obs_td.keys():
    ...
    for name, shape in zip(names, shapes):
        ...
        for i in range(dim):
            print(f"    idx {i:2d}: min={min_vals[i]:10.5f} | max={max_vals[i]:10.5f} | mean={mean_vals[i]:10.5f}")
        cursor += dim
