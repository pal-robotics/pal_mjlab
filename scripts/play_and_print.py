    ...
    def __call__(self, obs) -> torch.Tensor:
        ...
        if self.args.enable_yolo:
            ...
            with torch.no_grad():
                ...
                depth = camera_depth(self.inner_env, "head_realsense_camera", cutoff_distance=1.5)
            ...
            ee_pos_robot = quat_apply(quat_inv(self.robot.data.root_link_quat_w), ee_pos_w - self.robot.data.root_link_pos_w)
            ...
            ee_pose_i = (ee_pos_robot_cpu[0], ee_pos_robot_cpu[1], ee_pos_robot_cpu[2], ee_yaw_cpu)
            
            success_fit = False
            px, py, pz = 0.0, 0.0, 0.0
            theta = 0.0
            length, width, height = self.args.cube_size[0], self.args.cube_size[1], self.args.cube_size[2]
            ...
            if success_fit and self.is_grasped:
                ...
                if self.kf is not None:
                    ...
                    for kf, val in [(self.kf["kf_x"], px), (self.kf["kf_y"], py), (self.kf["kf_z"], pz)]:
                        if hasattr(kf, "x"):
                    ...
                    if hasattr(self.kf["kf_yaw"], "x"):
                        self.kf["kf_yaw"].x = np.array([math.cos(theta), math.sin(theta)], dtype=np.float32)
            ...
            if success_fit:
                if self.kf is None:
                    ...
                    if self.kf["yaw_locked"]:
                        ...
                        dist_moved = math.sqrt((curr_filtered_x - self.kf["yaw_lock_x"])**2 + (curr_filtered_y - self.kf["yaw_lock_y"])**2)
            ...
            est_yaw_r = torch.tensor([math.cos(theta), math.sin(theta)], device=device, dtype=torch.float32)
            ...
            print(f"  YOLO Fit: {'SUCCESS' if success_fit else 'FAIL'} | Grasped: {self.is_grasped}")
            print(f"  Pos Pred: [{px:.4f}, {py:.4f}, {pz:.4f}] | GT: [{gt_pos_r[0]:.4f}, {gt_pos_r[1]:.4f}, {gt_pos_r[2]:.4f}]")
            print(f"  Yaw Pred: {theta:.4f} rad ({math.degrees(theta):.2f}°) | GT: {gt_yaw_r:.4f} rad ({math.degrees(gt_yaw_r):.2f}°)")
        ...
        print(f"Object World Pos:  [{box_pos_w[0]:.4f}, {box_pos_w[1]:.4f}, {box_pos_w[2]:.4f}] m")
