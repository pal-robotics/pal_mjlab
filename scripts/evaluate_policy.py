...
try:
  ...
  print(
    "Warning: could not import filters from exchange workspace. Implementing fallback filters."
  )
  ...
  class ConstantVelocityKF:
    ...
    def update(self, z_pos, r_pos=None):
      ...
      self.P = (
        np.eye(2, dtype=np.float32) - np.outer(K.flatten(), H.flatten())
      ) @ self.P
      return True
...
def compute_angle_wrt_cube_lateral_surfaces(
  v: torch.Tensor, cube_quat: torch.Tensor
) -> torch.Tensor:
  """
  ...
  v_local_2d_norm = v_local_2d / torch.norm(v_local_2d, dim=-1, keepdim=True).clamp(
    min=1e-6
  )
...
def evaluate(args):
  ...
  if not os.path.exists(checkpoint_path):
    print(
      f"Error: Checkpoint path '{checkpoint_path}' does not exist!", file=sys.stderr
    )
    sys.exit(1)
  ...
  _noise_slices = (
    _build_noise_slices(env.observation_manager) if args.inject_noise else []
  )
  ...
  current_episode_grasp_and_lift = torch.zeros(
  ...
  assert len(fingertip_site_names) == 2, (
    f"Expected exactly 2 fingertip sites, found {len(fingertip_site_names)}"
  )
  ...
  left_global_idx = mujoco.mj_name2id(
    env.sim._mj_model, mujoco.mjtObj.mjOBJ_SITE, f"robot/{fingertip_site_names[0]}"
  )
  if left_global_idx == -1:
    left_global_idx = mujoco.mj_name2id(
      env.sim._mj_model, mujoco.mjtObj.mjOBJ_SITE, fingertip_site_names[0]
    )
  right_global_idx = mujoco.mj_name2id(
    env.sim._mj_model, mujoco.mjtObj.mjOBJ_SITE, f"robot/{fingertip_site_names[1]}"
  )
  if right_global_idx == -1:
    right_global_idx = mujoco.mj_name2id(
      env.sim._mj_model, mujoco.mjtObj.mjOBJ_SITE, fingertip_site_names[1]
    )
  ...
  # Find the grasp site
  grasp_site_idx, _ = robot.find_sites(
    ["gripper_right_grasping_site"], preserve_order=True
  )
  grasp_idx = grasp_site_idx[0]
  grasp_global_idx = mujoco.mj_name2id(
    env.sim._mj_model, mujoco.mjtObj.mjOBJ_SITE, "robot/gripper_right_grasping_site"
  )
  if grasp_global_idx == -1:
    grasp_global_idx = mujoco.mj_name2id(
      env.sim._mj_model, mujoco.mjtObj.mjOBJ_SITE, "gripper_right_grasping_site"
    )

  print(
    f"Evaluating {args.num_episodes} total episodes with {num_envs} parallel environments..."
  )
  step_count = 0
  ...
  while episodes_collected < args.num_episodes:
    with torch.no_grad():
      ...
      top_collision_float = top_surface_penetration_term(
        env=env, command_name="lift_height", threshold=0.0005
      )
    ...
    current_episode_min_pos_error = torch.minimum(
      current_episode_min_pos_error, position_error
    )
    ...
    activated_now = (
      contact_both
      & ~prev_contact_both
      & ~torch.tensor(contact_both_activated, device=device)
    )
    ...
    if args.enable_yolo:
      ...
      rgb_cpu = (
        (rgb.cpu().permute(0, 2, 3, 1) * 255.0).clip(0, 255).to(torch.uint8).numpy()
      )
      ...
      ee_pos_robot = quat_apply(
        quat_inv(robot.data.root_link_quat_w), ee_pos_w - robot.data.root_link_pos_w
      )
      ...
      dt = (
        ...
        else (env.sim.model.opt.timestep * env.cfg.decimation)
      )
      ...
      for i in range(num_envs):
        ...
        # ee_pose_i = (
        #   ee_pos_robot_cpu[i, 0],
        #   ee_pos_robot_cpu[i, 1],
        #   ee_pos_robot_cpu[i, 2],
        #   ee_yaw_cpu[i],
        # )
        ...
        if best_box is not None:
          ...
          if (x2 - x1) > 1 and (y2 - y1) > 1:
            ...
            valid_mask = (
              (depth_crop > 0.1) & (depth_crop < 1.5) & np.isfinite(depth_crop)
            )
            ...
            if rgb_mask is not None and np.sum(rgb_mask > 0) >= 5:
              ...
              if np.sum(cube_depth_mask) >= 5:
                ...
                inlier_mask = cube_depth_mask & (
                  np.abs(depth_crop - median_depth) < 0.05
                )
            ...
            if inlier_mask is not None and np.sum(inlier_mask) >= 5:
              ...
              if len(valid_depths) >= 5:
                ...
                pos_world = cam_pos_i + np.dot(
                  pos_cam * np.array([1.0, -1.0, -1.0]), cam_xmat_i.T
                )
        ...
        if is_grasped[i]:
          ...
          if success_fit:
            if kfs[i] is None:
              ...
              if kfs[i]["yaw_locked"]:
                ...
                dist_moved = math.sqrt(
                  (curr_filtered_x - kfs[i]["yaw_lock_x"]) ** 2
                  + (curr_filtered_y - kfs[i]["yaw_lock_y"]) ** 2
                )
            ...
            if kfs[i] is not None:
              ...
              _, _, robot_yaw_w = euler_xyz_from_quat(
                torch.tensor(robot_quat, device=device).unsqueeze(0)
              )
    ...
    if _noise_slices:
      ...
      for name, s, e, half in _noise_slices:
        ...
        if args.enable_yolo and name in ["object_position", "object_yaw"]:
          continue
        actor_obs[:, s:e] += (
          torch.rand(num_envs, e - s, device=device) * 2.0 - 1.0
        ) * half
    ...
    with torch.no_grad():
      if args.enable_yolo:
        ...
        v_squeeze_robot = quat_apply(
          quat_inv(robot.data.root_link_quat_w), v_squeeze_norm
        )
        # Compute angle wrt YOLO estimated orientation in robot base frame
        squeeze_angles = compute_angle_wrt_cube_lateral_surfaces(
          v_squeeze_robot, est_quat_r
        )
      else:
        squeeze_angles = compute_angle_wrt_cube_lateral_surfaces(
          v_squeeze_norm, cube_quat
        )
    ...
    if step_count % 10 == 0:
      print(
        f"[Step {step_count:4d}] Episode Progress: {episodes_collected:3d}/{args.num_episodes:3d} completed ({(episodes_collected / args.num_episodes) * 100.0:5.1f}%)"
      )
  ...
  mean_squeeze = (
    np.mean(angles_trigger_squeeze) if angles_trigger_squeeze else float("nan")
  )
  std_squeeze = (
    np.std(angles_trigger_squeeze) if angles_trigger_squeeze else float("nan")
  )
  ...
  print(
    f"Obs Noise Injection:   {'ENABLED (training-equivalent)' if args.inject_noise else 'disabled'}"
  )
  print("-" * 80)
  print(
    f"Success Rate:          {success_rate:.2f}% ({successful_runs}/{total_episodes})"
  )
  print(
    f"Grasp & Lift (≥1cm):   {total_grasp_and_lift} episodes ({grasp_and_lift_rate:.2f}%)"
  )
  print(
    f"Top Surface Collisions: {total_top_collisions} episodes ({collision_rate:.2f}%)"
  )
  ...
  print(f"  {'-' * 10}-+-{'-' * 8}-+-{'-' * 7}")
  for t, cnt, rate in zip(
    THRESHOLDS_M, threshold_counts, threshold_rates, strict=False
  ):
    print(f"  {t * 100:>9.0f}cm | {cnt:>8d} | {rate:>6.2f}%")
  print("-" * 80)
  print("Fingertip Alignment (Instant Before Contact):")
  print(
    f"  Squeeze Axis Angle:  Mean = {mean_squeeze:6.2f}° | Std = {std_squeeze:5.2f}°"
  )
  print("=" * 80 + "\n")
...
def main():
  parser = argparse.ArgumentParser(
    description="Run policy evaluation on 100 episodes and record key metrics."
  )
  parser.add_argument(
    "--checkpoint",
    type=str,
