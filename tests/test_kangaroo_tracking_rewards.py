...
class command_manager:
  def __init__(self, env):
    ...
    dummy_motion_command.anchor_lin_vel_w = torch.zeros((env.num_envs,3), device=env.device)
    dummy_motion_command.anchor_lin_vel_w[:, 2] += 2.0

    dummy_motion_command.robot_anchor_lin_vel_w = torch.zeros((env.num_envs,3), device=env.device)
...
def test_tracking_feet_air_time_reward (mock_env):
    ...
    env.scene["contact_sensor"].data.current_air_time = torch.zeros((env.num_envs, 2,), device= env.device)
    env.scene["contact_sensor"].data.current_air_time[:, 0] += 0.2
    env.scene["contact_sensor"].data.current_air_time[:, 1] += 0.4

    value = pal_mjlab_r.feet_air_time(
        env, sensor_name= "contact_sensor", threshold= 0.5
    )
    ...
    value = pal_mjlab_r.feet_air_time(
        env, sensor_name= "contact_sensor", threshold= 0.3
    )
    ...
    value = pal_mjlab_r.feet_air_time(
        env, sensor_name= "contact_sensor", threshold= 0.1
    )
    ...
    assert value[0] == pytest.approx(0.3, abs=1e-6), (
        tensor_value_error_message(test_name)
    )