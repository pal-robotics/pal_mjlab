...
class ExportableActor(nn.Module):
    def __init__(self, state_dict):
        ...
        # Build MLP
        mlp_state = {k.replace('mlp.', ''): v for k, v in state_dict.items() if k.startswith('mlp.')}
        indices = sorted(list(set(int(k.split('.')[0]) for k in mlp_state.keys() if k[0].isdigit())))
...
def export_rsl_rl_policy(pt_path: str, onnx_path: str, env, run_path: str):
    ...
    actor_state_dict = loaded_dict.get('actor_state_dict', loaded_dict.get('model_state_dict', loaded_dict))
