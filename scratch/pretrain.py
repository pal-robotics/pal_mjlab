...
class KeypointDataset(Dataset):
  ...
  def __getitem__(self, idx):
    ...
    # Load Keypoints
    kps = torch.tensor(
      item["keypoints"], dtype=torch.float32
    )  # shape (num_keypoints, 2)

    # Load Visibility Mask
    visibility = torch.tensor(
      item.get("visibility", [True] * self.num_keypoints), dtype=torch.float32
    )
    ...
    if self.training:
      ...
      kps[:, 1] = kps[:, 1] + dy
      off_screen = (
        (kps[:, 0] < 0) | (kps[:, 0] > 128) | (kps[:, 1] < 0) | (kps[:, 1] > 128)
      )
...
class PolicyCNNBackbone(nn.Module):
  def __init__(self, num_keypoints=6):
    ...
    # 6D Pose branch
    self.pose_head = nn.Sequential(nn.Linear(64, 64), nn.ELU(), nn.Linear(64, 6))
...
class ConvNeXtBackbone(nn.Module):
  def __init__(self, num_keypoints=6):
    ...
    self.downsample = nn.Sequential(
      LayerNorm2d(32), nn.Conv2d(32, 64, kernel_size=2, stride=2)
    )
    self.stage2 = nn.Sequential(ConvNeXtBlock(64), ConvNeXtBlock(64), ConvNeXtBlock(64))
    # Keypoints head
...
def train():
  parser = argparse.ArgumentParser(
    description="Unified Supervised Offline Keypoints & 6D Pose Pretraining"
  )
  parser.add_argument(
    "--backbone",
    type=str,
  ...
  parser.add_argument(
    "--device", type=str, default=None, help="Device to train on (cuda or cpu)"
  )
  ...
  if not resolved_dirs:
    raise FileNotFoundError(
      "No dataset directories ('dataset' or 'dataset_val') found."
    )
  ...
  if val_size <= 0:
    print(
      f"Warning: Pool size is too small ({len(all_items)}). Training with {train_size} samples and no validation."
    )
  ...
  train_dataset = KeypointDataset(
    train_items, training=True, num_keypoints=args.num_keypoints
  )
  ...
  if has_val:
    val_dataset = KeypointDataset(
      val_items, training=False, num_keypoints=args.num_keypoints
    )
  ...
  print(
    f"Starting supervised offline training ({args.backbone.upper()}, {args.num_keypoints} keypoints + 6D Pose) on {device}..."
  )
  for epoch in range(args.epochs):
    ...
    if has_val:
      ...
      print(
        f"Epoch {epoch + 1:2d}/{args.epochs} | Train Loss: {avg_train_loss:.6f} (KPs: {avg_kps_loss:.6f}, Pose: {avg_pose_loss:.6f}) | Val Loss: {avg_val_loss:.6f} (KPs: {avg_val_kps_loss:.6f}, Pose: {avg_val_pose_loss:.6f})"
      )
      ...
      if avg_val_loss < best_val_loss:
        ...
        print(
          f"  -> Saved new best model with Val Loss {best_val_loss:.6f} to {save_path}!"
        )
    else:
      print(
        f"Epoch {epoch + 1:2d}/{args.epochs} | Train Loss: {avg_train_loss:.6f} (KPs: {avg_kps_loss:.6f}, Pose: {avg_pose_loss:.6f})"
      )
      if avg_train_loss < best_train_loss:
        best_train_loss = avg_train_loss
        torch.save(model.state_dict(), save_path)
        print(
          f"  -> Saved new best model with Train Loss {best_train_loss:.6f} to {save_path}!"
        )
