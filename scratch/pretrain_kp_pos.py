...
class KeypointPositionDataset(Dataset):
  ...
  def __getitem__(self, idx):
    ...
    kps = torch.tensor(
      item["keypoints"][: self.num_keypoints], dtype=torch.float32
    )  # [4, 2]
    visibility = torch.tensor(
      item.get("visibility", [True] * 6)[: self.num_keypoints], dtype=torch.float32
    )
    ...
    if self.training:
      ...
      if zoom > 1.0:
        ...
        img_tensor = F.interpolate(
          img_tensor.unsqueeze(0), size=(H, W), mode="bilinear", align_corners=False
        ).squeeze(0)
        ...
        off_screen = (
          (kps[:, 0] < 0) | (kps[:, 0] > W) | (kps[:, 1] < 0) | (kps[:, 1] > H)
        )
      ...
      # E. Gaussian Noise
      if random.random() < 0.5:
        img_tensor = img_tensor + torch.randn_like(img_tensor) * random.uniform(
          0.005, 0.02
        )
...
class ConvNeXtKPPos(nn.Module):
  ...
  def __init__(self, num_keypoints=4):
    ...
    # 2. 3D Position Head
    self.pos_head = nn.Sequential(nn.Linear(64, 64), nn.ELU(), nn.Linear(64, 3))
...
def train():
  parser = argparse.ArgumentParser(
    description="Supervised Pretraining: 4 Cube Keypoints + 3D Position"
  )
  parser.add_argument(
    "--backbone",
    type=str,
  ...
  print(
    f"Initialized {args.backbone.upper()} Model — {num_params:,} trainable params | device={device}"
  )
  ...
  for epoch in range(args.epochs):
    ...
    if has_val:
      ...
      if avg_val_loss < best_val_loss:
        ...
        print(
          f"  → Saved new best model to {save_path} (Val Loss: {best_val_loss:.6f})"
        )
      ...
      if avg_train_loss < best_train_loss:
        ...
        torch.save(model.state_dict(), save_path)
        print(
          f"  → Saved new best model to {save_path} (Train Loss: {best_train_loss:.6f})"
        )
