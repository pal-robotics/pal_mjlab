...
def run_dataset_vis(args):
    ...
    print(f"Loaded {len(labels)} samples. Visualizing random {min(num_samples, len(labels))} samples...")
...
def run_model_predict_vis(args):
    ...
    if os.path.exists(model_path):
        ...
        if isinstance(checkpoint, dict) and 'actor_state_dict' in checkpoint:
            ...
            backbone_sd = {k[12:]: v for k, v in policy_sd.items() if k.startswith("cnns.camera.")}
            if backbone_sd:
        ...
        print(f"WARNING: Backbone path {model_path} not found! Model will run with random weights.")
    ...
    for i in range(num_steps):
        ...
        if i % save_every == 0:
            with torch.no_grad():
                ...
                print(f"Step {i} Predicted Keypoints: min={kps_np.min():.4f}, max={kps_np.max():.4f}")
...
def run_task_mlp_vis(args):
    ...
    if os.path.exists(model_path):
        ...
        print(f"WARNING: Checkpoint {model_path} not found! Running model with random weights.")
...
def main():
    ...
    parser.add_argument("--model_path", type=str, default=None, help="Path to checkpoint weights/pth file")
    parser.add_argument("--output_dir", type=str, default=None, help="Output directory to save images/video")
    parser.add_argument("--duration", type=float, default=2.0, help="Simulation duration in seconds (default: 2.0)")
    parser.add_argument("--interval", type=float, default=0.1, help="Time interval between saved frames (default: 0.1)")
    parser.add_argument("--num_keypoints", type=int, default=6, help="Number of keypoints (default: 6)")
    parser.add_argument("--device", type=str, default="cpu", help="Computation device (default: cpu)")
    
    # Dataset Mode Specific
    parser.add_argument("--data_dir", type=str, default="dataset_val", help="Dataset directory to sample from")
    parser.add_argument("--num_samples", type=int, default=100, help="Number of dataset frames to visualize")
    
    # Noisy GT Specific
    parser.add_argument("--noise_std", type=float, default=0.015, help="Standard deviation of added noise (default: 0.015)")
