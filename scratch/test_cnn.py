...
from pretrain import ConvNeXtBackbone, PolicyCNNBackbone


def test():
  parser = argparse.ArgumentParser(
    description="Test trained dual-head backbone on RGB dataset"
  )
  parser.add_argument(
    "--backbone",
    type=str,
  ...
  for i, idx in enumerate(indices):
    ...
    input_tensor = (
      torch.from_numpy(img_arr).permute(2, 0, 1).float().unsqueeze(0).to(device)
    )
    ...
    print(
      f"Sample {i:02d} | Position Error: {pos_err * 100:.2f} cm | Orientation Error: {rot_err:.2f}°"
    )
