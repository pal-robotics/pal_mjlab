...
from pal_mjlab.rl.flash_sac.base_agent import BaseAgent
from pal_mjlab.rl.flash_sac.network import (
    FlashSACActor,
    FlashSACDoubleCritic,
    FlashSACTemperature,
)
from pal_mjlab.rl.flash_sac.update import (
    update_actor,
    update_critic,
    update_target_network,
    update_temperature,
)
from pal_mjlab.rl.flash_sac.utils.network import Network
from pal_mjlab.rl.flash_sac.utils.reward_normalization import RewardNormalizer
from pal_mjlab.rl.flash_sac.utils.scheduler import warmup_cosine_decay_scheduler
from pal_mjlab.rl.flash_sac.buffers.torch_buffer import TorchUniformBuffer
from pal_mjlab.rl.flash_sac.types import NDArray, Tensor
...
class FlashSACAgent(BaseAgent[FlashSACConfig]):
    def __init__(
        ...
        if cfg.asymmetric_observation:
            self._actor_observation_dim = int(env_info["actor_observation_size"][0])
        else:
            self._actor_observation_dim = self._critic_observation_dim

        assert self._actor_observation_dim > 0, (
            f"Invalid actor_observation_dim={self._actor_observation_dim}. "
            f"env_info={env_info}"
        )
    ...
    def sample_actions(
        ...
        observations = torch.as_tensor(observations, dtype=torch.float32, device=self._device)

        obs_dim = observations.shape[-1]

        if obs_dim < self._actor_observation_dim:
            pad = self._actor_observation_dim - obs_dim
            observations = torch.cat(
                [observations, torch.zeros(observations.shape[0], pad, device=observations.device)],
                dim=-1,
            )
        elif self._cfg.asymmetric_observation:
    ...
    def load(self, path: str) -> None:
        ...
        if load_optimizer:
            ...
            if "grad_scaler" in agent_state:
                self._grad_scaler.load_state_dict(agent_state["grad_scaler"])
