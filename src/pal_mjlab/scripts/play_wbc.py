...
def run_play(task_id: str, cfg: PlayConfig):
  ...
  is_wbc_task = "motion" in env_cfg.commands and isinstance(
    env_cfg.commands["motion"], MotionCommandCfg
  )

  if is_wbc_task and cfg._demo_mode:
  ...
  if is_wbc_task:
    ...
    if cfg.motion_dir is not None and Path(cfg.motion_dir).exists():
      ...
      if cfg.motion_dir is not None:
        ...
        if cfg.wandb_run_path is None and cfg.checkpoint_file is not None:
          raise ValueError(
            "Tracking tasks require `motion_dir` when using `checkpoint_file`, "
            "or provide `wandb_run_path` so the motion artifact can be resolved."
          )
        if cfg.wandb_run_path is not None:
          wandb_run = api.run(str(cfg.wandb_run_path))
          art = next(
            (a for a in wandb_run.used_artifacts() if a.name == "motions"), None
          )
          if art is None:
            raise RuntimeError("No motion artifact found in the run.")
          motion_cmd.motion_dir = str(Path(art.download()))
