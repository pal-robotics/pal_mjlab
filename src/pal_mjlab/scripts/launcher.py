...
# Idea:
# To have an interactive menu that makes it much easier to launch
# simulation and deploy policies without running and/or modifying
# bash commands
...
def save_training_snapshot(environment_name, job_name, extra_opts, description=""):
  ...
  with open(snapshot_file, "a") as f:
    ...
    f.write(
      f"options    : --env.scene.num-envs 4096 --agent.run-name {job_name} --agent.logger tensorboard --agent.save-interval 500 {extra_opts if extra_opts else ''}\n"
    )
...
def open_menu():
    ...
    def run_remote():
      ...
      try:
        ...
        if not pid_line.startswith("__REMOTE_PID="):
          raise RuntimeError(f"Failed to obtain remote PID: {pid_line}")
  ...
  tk.Label(left, text="PAL_MJLAB", font=title_font, bg=PANEL, fg=ACCENT).pack(
    anchor="w", padx=16, pady=(18, 4)
  )
  tk.Label(left, text="simulation launcher", font=label_font, bg=PANEL, fg=MUTED).pack(
  ...
  def run_tsp():
    ...
    tk.Label(win, text="Select tsp view:", font=label_font, bg=BG, fg=MUTED).pack(
  ...
  def run_tsp_remove():
    ...
    tk.Label(win, text="tsp Job ID:", font=label_font, bg=BG, fg=MUTED).pack(
    ...
    def confirm():
      ...
      if not job_id:
        append_to_console(
          consoles[selected_console.get()], "✗ No job id entered\n", "status"
        )
        return
      win.destroy()
      launch_process(
        f"tsp -r {job_id}", consoles[selected_console.get()], label=f"tsp-r:{job_id}"
      )
  ...
  def run_deploy():
    if not tasks:
      append_to_console(
        consoles[selected_console.get()],
        "✗ No tasks loaded, click 'List Tasks' first\n",
        "status",
      )
      return
    ...
    if not checkpoints:
      append_to_console(
        consoles[selected_console.get()], "✗ No checkpoints found\n", "status"
      )
      return
    ...
    tk.Label(win, text="Select task to deploy", font=label_font, bg=BG, fg=MUTED).pack(
      anchor="w", padx=20, pady=(20, 6)
    )
    ...
    tk.Label(
      win, text="Select latest checkpoint", font=label_font, bg=BG, fg=MUTED
    ...
    def confirm():
      ...
      win.destroy()
      launch_process(cmd, consoles[selected_console.get()], label="deploy")
  ...
  def run_train():
    ...
    # Experiment name
    tk.Label(win, text="Experiment Name:", font=label_font, bg=BG, fg=MUTED).pack(
      anchor="w", padx=20, pady=(20, 6)
    )
    ...
    # Custom job name
    tk.Label(win, text="Custom Job Name:", font=label_font, bg=BG, fg=MUTED).pack(
    ...
    tk.Label(win, text="Select Environment:", font=label_font, bg=BG, fg=MUTED).pack(
      anchor="w", padx=20, pady=(0, 6)
    )
    selected_task = tk.StringVar(value=tasks[0] if tasks else "")
    dropdown_task = tk.OptionMenu(win, selected_task, *tasks)
    ...
    tk.Label(options_header, text="Options:", font=label_font, bg=BG, fg=MUTED).pack(
      side="left"
    )
    ...
    def run_load_default():
      ...
      if not task:
        append_to_console(
          consoles[selected_console.get()], "✗ No task selected\n", "status"
        )
        return
      try:
        ...
        append_to_console(
          consoles[selected_console.get()],
          f"✗ Failed to load default cfg for '{task}': {e}\n",
          "status",
        )
        return
      ...
      if not lines:
        append_to_console(
          consoles[selected_console.get()],
          f"✗ No default rewards found for '{task}'\n",
          "status",
        )
        return
    ...
    # Description label
    tk.Label(win, text="Description:", font=label_font, bg=BG, fg=MUTED).pack(
      anchor="w", padx=20, pady=(0, 6)
    )
    # Frame to hold Text + scrollbar
  ...
  def run_sync_checkpoints():
    ...
    tk.Label(
      win, text="Run name filter (substring match):", font=label_font, bg=BG, fg=MUTED
    ).pack(anchor="w", padx=20, pady=(10, 6))
    filter_entry = tk.Entry(win, font=label_font)
    filter_entry.pack(fill="x", padx=20)

    status_label = tk.Label(
      win, text="", font=label_font, bg=BG, fg=MUTED, wraplength=470, justify="left"
    )
  ...
  def setup_remote():
    ...
    def add_label(text):
      tk.Label(win, text=text, font=label_font, bg=BG, fg=MUTED).pack(
        anchor="w", padx=20, pady=(14, 6)
      )

    def add_entry(show=None):
      e = (
        tk.Entry(win, font=label_font, show=show)
        if show
        else tk.Entry(win, font=label_font)
      )
    ...
    def confirm():
      ...
      if stdout.read().decode() == "False\n":
        append_to_console(
          consoles[selected_console.get()],
          f"[WARNING] Directory {remote_path} does not exist in the remote PC !\n",
          "status",
        )
      else:
        write_remote_setup(user, ip, remote_path)
        remote_setup = user, ip, remote_path, 1  # noqa: F841
        append_to_console(
          consoles[selected_console.get()],
          f"Remote directory {remote_path} set\n",
          "status",
        )
  ...
  def clear_selected_terminal():
    ...
    append_to_console(
      consoles[selected_console.get()],
      f"Terminal {selected_console.get() + 1} ready.\n\n",
      "status",
    )
  ...
  tk.Label(left, text="Select Terminal:", font=label_font, bg=PANEL, fg=MUTED).pack(
  ...
  tk.Label(left, text="Job Queue (tsp):", font=label_font, bg=PANEL, fg=MUTED).pack(
  ...
  tk.Label(left, text="Running Processes:", font=label_font, bg=PANEL, fg=MUTED).pack(
    anchor="w", padx=16, pady=(16, 4)
  )

  proc_listbox = tk.Listbox(
    left, font=label_font, bg=BG, fg=TEXT, selectbackground=ACCENT2, height=6
  )
  ...
  for i in range(4):
    ...
    c_widget.bind(
      "<Key>",
      lambda e: "break"
      if not (e.state & 0x4 and e.keysym in ("c", "C", "a", "A"))
      else None,
    )
