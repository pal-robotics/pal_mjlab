...
from pal_mjlab.tasks.lift.lift_env_cfg import make_lift_env_cfg
from pal_mjlab.tasks.lift.mdp import LiftingCommandCfg
import mujoco
import random

# ADD cube 
def get_cube_spec(cube_size: float = 0.020, mass: float = 0.10) -> mujoco.MjSpec:
  spec = mujoco.MjSpec()
  ...
  body.add_geom(
    ...
    friction=(0.8, 0.8, 0.0001),
  )
  return spec

def pal_tiago_lift_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
    """Create PAL Robotics TIAGo lift configuration."""
