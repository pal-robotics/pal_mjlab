...
class VisionCurriculumRunner(ManipulationOnPolicyRunner):
        ...
        if hasattr(self.alg, model_attr):
          ...
          if model_attr == "actor_critic":
            for sub_model in [obj.actor, obj.critic]:
              ...
              print(
                f"[Curriculum] Loaded into actor_critic.{sub_model.__class__.__name__}. Missing keys: {len([mk for mk in missing if mk.startswith('cnns')])}, Unexpected keys: {len(unexpected)}"
              )
            loaded = True
          else:
            missing, unexpected = obj.load_state_dict(mapped_weights, strict=False)
            print(
              f"[Curriculum] Loaded into {model_attr}. Missing keys: {len([mk for mk in missing if mk.startswith('cnns')])}, Unexpected keys: {len(unexpected)}"
            )
            loaded = True
      if not loaded:
        print(
          "[Curriculum] ERROR: Could not locate actor or critic models in algorithm!"
        )
    else:
      print(
        "[Curriculum] WARNING: No pre-trained backbone found at 'pretrained_backbone.pth'. Starting from random weights."
      )
  ...
  def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False):
    ...
    if self.current_learning_iteration < unfreeze_at:
      print(
        f"[Curriculum] Initializing Phase 1: CNN backbone is FROZEN until iteration {unfreeze_at}."
      )
      self._set_cnn_grad(False)
    else:
      print("[Curriculum] Resuming Phase 2: CNN backbone is already UNFROZEN.")
  ...
  def _set_cnn_grad(self, requires_grad: bool):
    for model in [self.alg.actor, self.alg.critic]:
      if hasattr(model, "cnns"):
        ...
        print(
          f"[WARN] Could not find 'cnns' in {model.__class__.__name__}. Skipping grad update."
        )
