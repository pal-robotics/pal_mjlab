...
class VisionPretrainedRunner(ManipulationOnPolicyRunner):
  ...
  def __init__(self, env, train_cfg, log_dir=None, device="cpu"):
    ...
    print(
      f"[VisionPretrainedRunner] Initializing in '{self.backbone_mode.upper()}' mode."
    )
    ...
    backbone_path = (
      "pretrained_convnext.pth" if is_convnext else "pretrained_backbone.pth"
    )
    if not os.path.exists(backbone_path):
      backbone_path = (
        "pretrained_backbone.pth" if is_convnext else "pretrained_convnext.pth"
      )

    # 3. Load pre-trained visual backbone weights
    if os.path.exists(backbone_path):
      print(
        f"[VisionPretrainedRunner] Loading pre-trained backbone weights from {backbone_path}..."
      )
        ...
        if hasattr(self.alg, model_attr):
          ...
          if model_attr == "actor_critic":
            for sub_model in [obj.actor, obj.critic]:
              missing, unexpected = sub_model.load_state_dict(
                mapped_weights, strict=False
              )
              print(
      ...
      if not loaded:
        print(
          "[VisionPretrainedRunner] ERROR: Could not locate actor or critic models in algorithm!"
        )
    else:
      print(
        f"[VisionPretrainedRunner] WARNING: No pre-trained backbone found at '{backbone_path}'. Starting from random weights."
      )
  ...
  def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False):
    if self.backbone_mode == "frozen":
      print(
        "[VisionPretrainedRunner] Starting PPO training with PERMANENTLY FROZEN visual backbone."
      )
    elif self.backbone_mode == "curriculum":
      print(
        f"[VisionPretrainedRunner] Starting PPO curriculum training (unfreeze at iteration {self.unfreeze_at})."
      )
  ...
  def _set_cnn_grad(self, requires_grad: bool):
    ...
    for model in [self.alg.actor, self.alg.critic]:
      if hasattr(model, "cnns"):
        ...
        print(
          f"[VisionPretrainedRunner] WARNING: Could not find 'cnns' in {model.__class__.__name__}. Skipping."
        )
    print(
      f"[VisionPretrainedRunner] Set requires_grad={requires_grad} for {frozen_count} backbone parameters."
    )
