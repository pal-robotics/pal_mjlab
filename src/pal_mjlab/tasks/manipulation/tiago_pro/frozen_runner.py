        ...
        if hasattr(self.alg, model_attr):
          ...
          if model_attr == "actor_critic":
            for sub_model in [obj.actor, obj.critic]:
              ...
              print(
                f"[Frozen Runner] Loaded into actor_critic.{sub_model.__class__.__name__}. Missing keys: {len([mk for mk in missing if mk.startswith('cnns')])}, Unexpected keys: {len(unexpected)}"
              )
            loaded = True
          else:
            missing, unexpected = obj.load_state_dict(mapped_weights, strict=False)
            print(
              f"[Frozen Runner] Loaded into {model_attr}. Missing keys: {len([mk for mk in missing if mk.startswith('cnns')])}, Unexpected keys: {len(unexpected)}"
            )
            loaded = True
      if not loaded:
        print(
          "[Frozen Runner] ERROR: Could not locate actor or critic models in algorithm!"
        )
    else:
      print(
        "[Frozen Runner] WARNING: No pre-trained backbone weights found. Starting from random weights."
      )
  ...
  def _freeze_backbone(self):
    ...
    for model in [self.alg.actor, self.alg.critic]:
      if hasattr(model, "cnns"):
        ...
        print(
          f"[Frozen Runner] WARNING: Could not find 'cnns' in {model.__class__.__name__}. Skipping."
        )
    print(
      f"[Frozen Runner] Success: Permanently FROZE {frozen_count} backbone parameters."
    )

  def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False):
    print(
      "[Frozen Runner] Starting training with PERMANENTLY FROZEN visual backbone features."
    )
    return super().learn(
