...
class UniformVelocityCommand(CommandTerm):
    ...
    def _debug_vis_impl(self, scn: mujoco.MjvScene) -> None:
        ...
        for batch in range(self.num_envs):
            ...
            def add_arrow(
                from_w, to_w, rgba, width=0.015, size=(0.005, 0.02, 0.02)
            ):
                scn.ngeom += 1
            ...
            add_arrow(
                *make_arrow(cmd_lin_from, cmd_lin_to),
                rgba=[0.2, 0.2, 0.6, 0.6],
            )
            add_arrow(
                *make_arrow(cmd_ang_from, cmd_ang_to),
                rgba=[0.2, 0.6, 0.2, 0.6],
            )

            act_lin_from = np.array([0, 0, z_offset]) * scale
            act_lin_to = (
                act_lin_from
                + np.array([lin_vel_b[0], lin_vel_b[1], 0]) * scale
            )
            act_ang_from = act_lin_from
            act_ang_to = act_ang_from + np.array([0, 0, ang_vel_b[2]]) * scale
            add_arrow(
                *make_arrow(act_lin_from, act_lin_to),
                rgba=[0.0, 0.6, 1.0, 0.7],
            )
            add_arrow(
                *make_arrow(act_ang_from, act_ang_to),
                rgba=[0.0, 1.0, 0.4, 0.7],
            )
